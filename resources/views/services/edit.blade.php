                    ...
                    <form method="POST" action="{{ route('services.update', $service) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="hsn" type="text" name="hsn" :value="old('hsn', $service->hsn)" class="mt-1 block w-full" />
                            ...
                            <div class="grid grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="cgst_rate" type="number" name="cgst_rate" :value="old('cgst_rate', $service->cgst_rate)" class="mt-1 block w-full" min="0" max="100" step="0.01" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('cgst_rate')" />
                                </div>

                                <div>
                                    <x-input-label for="sgst_rate" :value="__('SGST Rate (%)')" />
                                    <x-text-input id="sgst_rate" type="number" name="sgst_rate" :value="old('sgst_rate', $service->sgst_rate)" class="mt-1 block w-full" min="0" max="100" step="0.01" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('sgst_rate')" />
                                </div>

                                <div>
                                    <x-input-label for="igst_rate" :value="__('IGST Rate (%)')" />
                                    <x-text-input id="igst_rate" type="number" name="igst_rate" :value="old('igst_rate', $service->igst_rate)" class="mt-1 block w-full" min="0" max="100" step="0.01" required />
